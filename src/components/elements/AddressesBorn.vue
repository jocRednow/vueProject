...
export default {
  ...
  computed: {
    ...mapState('bornPage', ['serverAddressRequest', 'serverSubRequest', 'serverCountriesRequest',
  ...
  methods: {
    ...
    searchAddressOnServer (search) {
      ...
      if (search.length >= lettersLimit) {
        this.$store.dispatch('bornPage/onSearchAddressData', {'level': null, 'refId': null, 'name': search})
      }
    },
    selectAddress (options, type) {
      this.$store.dispatch('bornPage/onSearchAddressDataParams', {'options': options, 'type': type})
    },
    selectHouseOrFlat (options, type) {
      this.$store.dispatch('bornPage/onSearchHouseOrFlatDataParams', {'dto': this.address, 'type': type})
    },
    searchSubOnServer ($event, obj) {
      let lettersLimit = 1
      if ($event.target.value.length >= lettersLimit) {
        this.$store.dispatch('bornPage/onSearchSubData', {'level': obj.level, 'dto': this.address, 'searchString': $event.target.value})
      }
    }
  }
}
