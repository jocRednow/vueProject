...
export default {
  name: 'addresses-perinatal-death',
  components: {
    CoolSelect
  },
  props: {
  ...
  computed: {
    ...mapState('perinatalDeathPage', ['serverAddressRequest', 'serverSubRequest', 'serverCountriesRequest', 'serverRegionRequest', 'serverCityRequest', 'serverTownRequest',
      'serverInnerTerritoryRequest', 'server65Request', 'server91Request', 'serverStreetRequest', 'serverHouseRequest']),
    ...mapState('deathPage', ['serverAdminRegionRequest', 'countriesList'])
  },
  methods: {
    clearAddress () {
    ...
    searchAddressOnServer (search) {
      ...
      if (search.length >= lettersLimit) {
        this.$store.dispatch('perinatalDeathPage/onSearchAddressData', {'level': null, 'refId': null, 'name': search})
      }
    },
    selectAddress (options, type) {
      this.$store.dispatch('perinatalDeathPage/onSearchAddressDataParams', {'options': options, 'type': type})
    },
    selectHouseOrFlat (options, type) {
      this.$store.dispatch('perinatalDeathPage/onSearchHouseOrFlatDataParams', {'dto': this.address, 'type': type})
    },
    searchSubOnServer ($event, obj) {
      let lettersLimit = 1
      if ($event.target.value.length >= lettersLimit) {
        this.$store.dispatch('perinatalDeathPage/onSearchSubData', {'level': obj.level, 'dto': this.address, 'searchString': $event.target.value})
      }
    }
  }
}
